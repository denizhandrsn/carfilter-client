import React from "react";
import { ToDoModel } from "../../models/toDoModel";

type Props = {
  todo: ToDoModel; //? zorunlu halde şuan todo?: ToDoModel; olsaydı opsiyonel olurdu
  onBtnClick: (id: number) => void;
};

const ToDoCard = (props: Props) => {
  return (
    <div className="col-span-3 mb-3 mr-3">
      <div className="max-w-sm rounded overflow-hidden shadow-lg">
        <div className="px-6 py-4">
          <div className="font-bold text-xl mb-2">{props.todo.title}</div>
          <p className="text-gray-700 text-base">{props.todo.id}</p>
        </div>
        <div className="px-6 pt-4 pb-2">
          <span className="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">
            {props.todo.completed ? (
              <span className="text-green-500">Tamamlandı</span>
            ) : (
              <span className="text-red-700">Tamamlanmadı</span>
            )}
          </span>
          <span>
            <button
              onClick={() => {
                props.onBtnClick(props.todo.id);
              }}
              className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded"
            >
              Tikla
            </button>
          </span>
        </div>
      </div>
    </div>
  );
};

export default ToDoCard;
